$.fn.datepicker = function (options) {
	...
	$(field).focus(function () {
	   ...
	   var focusLoad = setInterval(function () {
	      clearInterval(focusLoad);
	      var screenHeight = window.innerHeight; /* screen.height */
	      var pickerTop = parseInt(picker.position().top);
	      var pickerOffset = parseInt(picker.offset().top);
	      var pickerHeight = parseInt(picker.height());
	      var pickerAvailable = parseInt(screenHeight - pickerTop); pickerAvailable -= 5;
	      var marginTop = 0;
	      if (pickerAvailable < pickerHeight) {
	         marginTop = parseInt(pickerAvailable - pickerHeight); //marginTop -= 10;
	         $(picker).animate({ marginTop: marginTop + "px" }, 200);
	      }
	      /* console.log("datepicker-focus", { screenHeight: screenHeight, pickerOffset, pickerTop, pickerHeight, pickerAvailable, marginTop }); */
	   }, 500);
	});
	...
	function AppViewModel(field, picker, options) {
		...
		self.processDate = function(day) {
			if (day) {
				...
				$(self.field).val(dateString).trigger('change');
			}
		};
