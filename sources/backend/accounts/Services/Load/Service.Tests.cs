...
namespace Elesse.Accounts.Tests
{
   partial class AccountServiceTests
   {
      ...
      public async void Load_WithNullParameter_MustReturnBadResult()
      {
         ...
         Assert.Equal(Shared.Results.Message("accounts", Shared.enMessageType.Warning, WARNINGS.INVALID_LOAD_PARAMETER), (result.Result as BadRequestObjectResult).Value);
      }
